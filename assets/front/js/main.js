...
$(document).ready(function(){
    ...
    function onScrollAnim(items) {
        if (!('IntersectionObserver' in window)) {
            // Fallback: show everything immediately
            items.each(function() {
                $(this).addClass('animated').addClass($(this).attr('data-wpa-animation'));
            });
            return;
        }
        ...
        var observer = new IntersectionObserver(function(entries) {
            entries.forEach(function(entry) {
                if (entry.isIntersecting) {
                    var el = $(entry.target);
                    var wpaAnimation = el.attr('data-wpa-animation');
                    el.addClass('animated').addClass(wpaAnimation);
        ...
        items.each(function() {
            ...
            if ($(window).width() < 1024) {
                wpaElement.css({ 'animation-delay': '0s' });
            }
    ...
    // Scroll reveal for .reveal-on-scroll
    (function() {
    ...
    var waveSpeed = 0.2;
    $('.wave-path-1').wavify({
        height: 25, bones: 5, amplitude: 40, speed: waveSpeed,
        color: 'rgba(150, 97, 255, .8)'
     });
    $('.wave-path-2').wavify({ height: 25, bones: 6, amplitude: 40, speed: waveSpeed - 0.1 });
    $('.wave-path-3').wavify({ height: 25, bones: 7, amplitude: 40, speed: waveSpeed });
    $('.wave-path-4').wavify({ height: 25, bones: 8, amplitude: 40, speed: waveSpeed - 0.1 });
    $('.wave-path-5').wavify({ height: 25, bones: 6, amplitude: 40, speed: waveSpeed });
    $('.wave-path-6').wavify({ height: 25, bones: 6, amplitude: 40, speed: waveSpeed - 0.1 });
        ...
        var newsSwiper = new Swiper('#news-swiper',{
            ...
            on: {
                init: function(swiper){
                    var slidesOutsideEvenGrid = this.slides.length % this.params.slidesPerColumn;
                    if (slidesOutsideEvenGrid != 0) {
                        var slidesNeededToFillGrid = this.params.slidesPerColumn - slidesOutsideEvenGrid;
    ...
    // $anchor nav 步驟錨點 (smooth scroll native)
    $('.anchor-nav-box').each(function(){
        $(this).find('.anchor-item').each(function(){
            var thisStep = $(this).attr('data-step');
            $(this).on('click', function(){
                if( thisStep !== ''){
                    var target = document.querySelector('.section-step-hide[data-step="'+thisStep+'"]');
                    if (target) {
