(function($){$.fn.wavify=function(options){
var settings=$.extend({container:options.container?options.container:'body', height:200, amplitude:100, speed:.15, bones:3, color:'rgba(255,255,255, 0.20)'},options);var wave=this,width=$(settings.container).width(),height=$(settings.container).height(),points=[],lastUpdate,totalTime=0;
...
var redraw=debounce(function(){wave.attr('d','');points=[];totalTime=0;width=$(settings.container).width();height=$(settings.container).height();lastUpdate=false;setTimeout(function(){draw();},50);},250);$(window).on('resize',redraw);
return draw();};}(jQuery));